...
object MostObscureSuperhero {
  ...
  def main(args: Array[String]) {
    ...
    val mostObscure = totalFriendsByCharacter
      .filter{ case (_, friends) => friends==1 }
      .collect
    ...
    mostObscure.map( h=>namesRdd.lookup(h._1).mkString->h._2 )
      .foreach{ case (hero, count) =>println(s"$hero is very obscure with just $count co-appearances") }
